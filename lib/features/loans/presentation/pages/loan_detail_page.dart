...
class LoanGuarantorItem {
  ...
  const LoanGuarantorItem(
      {required this.memberName,
      required this.memberCode,
      required this.shareAmount});
  factory LoanGuarantorItem.fromJson(Map<String, dynamic> j) =>
      LoanGuarantorItem(
...
class LoanCollateralItem {
  ...
  const LoanCollateralItem(
      {required this.type,
      required this.description,
      required this.estimatedValue});
  factory LoanCollateralItem.fromJson(Map<String, dynamic> j) =>
      LoanCollateralItem(
...
class LoanDetail {
  ...
  factory LoanDetail.fromJson(Map<String, dynamic> j) => LoanDetail(
        ...
        processingFeePercent:
            (j['processingFeePercent'] as num?)?.toDouble() ?? 1,
...
final _loanDetailProvider =
    FutureProvider.autoDispose.family<LoanDetail, String>((ref, id) async {
...
final _loanScheduleProvider =
    FutureProvider.autoDispose.family<List<EmiRow>, String>((ref, id) async {
  final dio = ref.watch(dioProvider);
...
final _loanPaymentsProvider = FutureProvider.autoDispose
    .family<List<PaymentItem>, String>((ref, id) async {
  final dio = ref.watch(dioProvider);
  ...
  return raw.map((e) {
    ...
    return PaymentItem(
      ...
      date: DateTime.tryParse(m['paymentDate'] as String? ?? '') ??
          DateTime.now(),
...
class _LoanDetailPageState extends ConsumerState<LoanDetailPage>
  ...
  Future<void> _showApproveDialog(LoanDetail loan) async {
    final amtCtrl =
        TextEditingController(text: loan.appliedAmount.toStringAsFixed(0));
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                            child: Text(
                                'This will generate the full EMI repayment schedule.',
                                style: AppTextStyles.bodySmall
  ...
  Future<void> _showPaymentDialog(LoanDetail loan) async {
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Collect EMI Payment',
                                style: AppTextStyles.titleMedium),
                            Text(loan.loanNumber,
                                style: AppTextStyles.bodySmall
                                    .copyWith(color: AppColors.textSecondary)),
                              ...
                              children: [
                                ...
                                Text('EMI ${pendingEmi.no}',
                                    style: AppTextStyles.titleSmall.copyWith(
                                        color: pendingEmi.status == 'Overdue'
                                            ? AppColors.error
                                            : AppColors.primary,
                              ...
                              children: [
                                Text('Principal',
                                    style: AppTextStyles.bodySmall.copyWith(
                                        color: AppColors.textSecondary)),
                                Text(
                                    'NPR ${pendingEmi.principal.toStringAsFixed(0)}',
                                    style: AppTextStyles.bodySmall),
                              ]),
                              ...
                              children: [
                                Text('Interest',
                                    style: AppTextStyles.bodySmall.copyWith(
                                        color: AppColors.textSecondary)),
                                Text(
                                    'NPR ${pendingEmi.interest.toStringAsFixed(0)}',
                                    style: AppTextStyles.bodySmall),
                              ]),
                          const SizedBox(height: 2),
                          Row(
                      ...
                      Container(
                        ...
                        child: const Row(children: [
                          Icon(Icons.info_outline,
                              color: AppColors.warning, size: 16),
                          SizedBox(width: 8),
                          Text('No pending EMI — extra payment',
                              style: AppTextStyles.bodySmall),
                        ]),
                      ),
                    ],
    ...
    try {
      final dio = ref.read(dioProvider);
      final res =
          await dio.post('/api/v1/loans/${widget.loanId}/payment', data: {
        'amount': double.parse(amtCtrl.text.trim()),
        'paymentMode': mode,
        'paymentDate': DateTime.now().toUtc().toIso8601String(),
        'narration': narrationCtrl.text.trim().isEmpty
            ? null
      ...
      final balance =
          (respData?['outstandingBalance'] as num?)?.toDouble() ?? 0;
              ...
              Positioned(
                ...
                child: Container(
                  padding: const EdgeInsets.fromLTRB(AppDimensions.md,
                      AppDimensions.sm, AppDimensions.md, AppDimensions.lg),
                  decoration: BoxDecoration(
                  ...
                  Column(
                      ...
                      children: [
                        Row(
                            ...
                            children: [
                              Text(
                                  'Outstanding: ${_fmtAmt(loan.outstandingBalance)}',
                                  style: AppTextStyles.bodySmall
                                      .copyWith(color: Colors.white70)),
                              Text(
                                  '${(loan.paidPercent * 100).toStringAsFixed(1)}% paid',
      ...
      children: [
        ...
        _InfoSection(title: 'Loan Details', rows: [
          ...
          if (loan.approvedAmount != null)
            InfoRow(
                label: 'Approved Amount', value: _fmtAmt(loan.approvedAmount!)),
          if (loan.disbursedAmount != null)
            InfoRow(
                label: 'Disbursed Amount',
                value: _fmtAmt(loan.disbursedAmount!)),
          InfoRow(
              label: 'Processing Fee', value: '${loan.processingFeePercent}%'),
          InfoRow(
              label: 'Interest Rate',
              value: '${loan.interestRate}% p.a. (${loan.interestType})'),
          ...
          if (loan.appliedDate != null)
            InfoRow(label: 'Applied Date', value: loan.appliedDate!),
          if (loan.approvedDate != null)
            InfoRow(label: 'Approved Date', value: loan.approvedDate!),
          if (loan.disbursedDate != null)
            InfoRow(label: 'Disbursed Date', value: loan.disbursedDate!),
          if (loan.closedDate != null)
            InfoRow(label: 'Closed Date', value: loan.closedDate!),
        ...
        _InfoSection(title: 'Outstanding Summary', rows: [
          InfoRow(
              ...
              value: _fmtAmt(loan.outstandingBalance)),
          InfoRow(label: 'EMI Amount', value: _fmtAmt(loan.emiAmount)),
          InfoRow(
              label: 'Overdue Amount',
              value:
                  loan.overdueAmount > 0 ? _fmtAmt(loan.overdueAmount) : '—'),
          InfoRow(
              label: 'Overdue Days',
              value: loan.overdueDays > 0 ? '${loan.overdueDays} days' : '—'),
          InfoRow(
              label: 'Installments Paid',
              value: '${loan.installmentsPaid} of ${loan.installmentsTotal}'),
          InfoRow(
              label: 'Installments Remaining',
              value: '${loan.installmentsTotal - loan.installmentsPaid}'),
        ]),
        ...
        if (loan.guarantors.isNotEmpty) ...[
          ...
          _InfoSection(title: 'Guarantors', rows: [
            for (int i = 0; i < loan.guarantors.length; i++) ...[
              InfoRow(
                  label: 'Guarantor ${i + 1}',
                  value: loan.guarantors[i].memberName),
              InfoRow(label: 'Code', value: loan.guarantors[i].memberCode),
              InfoRow(
                  label: 'Shares Pledged',
                  value: _fmtAmt(loan.guarantors[i].shareAmount)),
            ]
          ]),
        ],
        ...
        if (loan.collaterals.isNotEmpty) ...[
          ...
          _InfoSection(title: 'Collateral', rows: [
            for (int i = 0; i < loan.collaterals.length; i++) ...[
              ...
              InfoRow(
                  label: 'Description', value: loan.collaterals[i].description),
              InfoRow(
                  label: 'Estimated Value',
                  value: _fmtAmt(loan.collaterals[i].estimatedValue)),
            ]
          ]),
        ],

        if (loan.approvalRemarks != null &&
            loan.approvalRemarks!.isNotEmpty) ...[
          const SizedBox(height: AppDimensions.md),
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text('No schedule yet',
              style: AppTextStyles.bodyMedium
                  .copyWith(color: AppColors.textSecondary)),
          Text('Loan may not be disbursed yet',
              style: AppTextStyles.bodySmall
                  .copyWith(color: AppColors.textSecondary)),
        ]),
      ),
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    final isCurrent = r.status == 'Overdue' ||
                        (!isPaid &&
                            i == rows.indexWhere((x) => x.status != 'Paid'));
                    return Container(
                      color: isCurrent
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                            child: Text(
                                ...
                                textAlign: TextAlign.center)),
                        Expanded(
                            child: Text(_fmtK(r.principal),
                                style: AppTextStyles.bodySmall,
                                textAlign: TextAlign.center)),
                        Expanded(
                            child: Text(_fmtK(r.interest),
                                style: AppTextStyles.bodySmall,
                                textAlign: TextAlign.center)),
                        Expanded(
                            child: Text(_fmtK(r.emi),
                                style: AppTextStyles.labelSmall,
                                textAlign: TextAlign.center)),
                        Expanded(
                            child: Text(isPaid ? '—' : _fmtK(r.balance),
                                style: AppTextStyles.bodySmall
                                    .copyWith(color: AppColors.textSecondary),
                                textAlign: TextAlign.center)),
                      ]),
                      ...
                      children: [
                        ...
                        Row(
                            ...
                            children: [
                              _MiniPair(
                                  label: 'Principal',
                                  value: _fmtAmt(p.principal)),
                              _MiniPair(
                                  label: 'Interest',
                                  value: _fmtAmt(p.interest)),
                              _MiniPair(
                                  label: 'Penalty',
                                  value:
                                      p.penalty > 0 ? _fmtAmt(p.penalty) : '—'),
                              _MiniPair(
                                  label: 'Total', value: _fmtAmt(p.total)),
                            ]),
                        const SizedBox(height: AppDimensions.sm),
